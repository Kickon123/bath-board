...
def update_temps(sensors: dict, gateway: dict | None = None) -> int:
    ...
    「古くなりすぎたら消す」判断はここでは行わない（CMS側は温度履歴の確認用に
    stale状態でも見え続けてほしいため）。実際の案内板でどこまで古い値を表示に
    使うかは、"at" と現在時刻の差を見て、案内板の描画側で判断する
    （詳細は shared/docs/DEVICE_AUTO_DISCOVERY.md 付近の経緯を参照）。
    戻り値は今回検出できたセンサー数。
    """
